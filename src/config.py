...
class DataArguments:
    ...
    prediction_horizon: int = field(
        default=16,
        metadata={"help": "Number of future frames to predict actions for (matches model output shape)."}
    )
    analog_deadzone: float = field(
        default=0.05,
        metadata={"help": "Threshold below which analog stick values are treated as 0.0."}
    )
